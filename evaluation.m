...
%             plot_intersection(clouds_path,layer,outlier_range, rangeLimit, viewing_angle,precision,center_statistics_,num_bins,reload,false);
%        
...
%         plot_intersection(clouds_path,layer,outlier_range, rangeLimit, viewing_angle,precision,center_statistics,num_bins_,reload,false);
%         
...
%         plot_intersection(clouds_path,layer,outlier_range, rangeLimit, viewing_angle_,precision,center_statistics,num_bins,reload,false);
%         
...
%     end
       ...
       %% nan angle size
    path = [plot_path,'nan_range_eval/'];
    
    parfor angle = 1:10
        outlier_range_ = deg2rad(angle);
        plot_intersection(clouds_path,layer,outlier_range_, rangeLimit, viewing_angle,precision,center_statistics,num_bins,reload,false);
        
        filename =sprintf('Isovists %d_%d bins_%s_angle_range %d.png',intersections,num_bins,center_statistics,angle);
        set(gcf, 'Position', get(0, 'Screensize'));
        mkdir(path)
        export_fig([path,filename],'-pdf','-m3','-transparent')
    end
